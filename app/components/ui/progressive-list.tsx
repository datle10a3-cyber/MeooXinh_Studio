...
export function useTabletTouchViewport() {
  const [matches, setMatches] = useState(isTabletTouchViewport);

  useEffect(() => {
    const query = window.matchMedia("(min-width: 768px) and (max-width: 1366px) and (pointer: coarse)");
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      if (entries.some((entry) => entry.isIntersecting)) {
        ...
        if (tabletTouch) {
          timer = window.setTimeout(loadMore, 120);
        } else {
          loadMore();
        }
      }
    }, { rootMargin: tabletTouch ? "120px 0px" : "600px 0px" });
