...
export function AlertModal({ isOpen, message, onClose, title }: AlertModalProps) {
  ...
  return createPortal(
    // No animate-in on container — just opacity via CSS class.
    // No backdrop-blur-xl on card — replaced with solid bg for tablet perf.
    // No zoom-in-95 / slide-in-from-bottom — causes paint during touch navigation.
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      {/* Backdrop — no backdrop-blur, use solid dark overlay instead */}
      <div
        className="absolute inset-0 bg-slate-900/50"
        onClick={onClose}
      />

      {/* Card — no animate-in, no backdrop-blur-xl, just solid white */}
      <div className="relative w-full max-w-sm overflow-hidden rounded-[2rem] border border-white bg-white p-6 text-center shadow-[0_24px_70px_rgba(184,95,108,0.22)]">
        ...
        <div className="mt-6">
          <Button
            className={cn(
              "h-12 w-full rounded-2xl text-sm font-black transition-colors",
              isSuccess
