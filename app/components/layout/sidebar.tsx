...
export const Sidebar = memo(function Sidebar({ session, rootAdminTheme = false }: { session: CurrentSession | null; rootAdminTheme?: boolean }) {
  ...
  function classes(active: boolean) {
    if (rootAdminTheme) {
      return cn(
        "flex h-10 w-full items-center justify-center gap-3 rounded-xl px-3 text-left text-sm font-bold transition-all duration-200 xl:justify-start",
  ...
  return (
    <aside className={cn("studio-sidebar studio-gpu hidden shrink-0 overflow-y-auto px-2 py-4 md:flex md:w-24 md:flex-col lg:w-28 xl:w-64 xl:px-4 xl:py-6", rootAdminTheme ? "border-r border-emerald-300/15 bg-[#04110A]" : "border-r border-[#F4C7C4]/50 bg-white/50")}>
      <div className={cn("mb-4 rounded-2xl p-2 shadow-[0_8px_20px_rgba(184,95,108,0.06)] xl:mb-6 xl:rounded-[2rem] xl:p-5", rootAdminTheme ? "border border-emerald-300/20 bg-[#06140D] text-slate-100 shadow-[0_18px_50px_rgba(2,6,23,0.28)]" : "border-2 border-[#F7AFC0] bg-white text-[#5B342C]")}>
        {rootAdminTheme ? (
        ...
        <p className={cn("mt-3 hidden rounded-full px-3 py-2 text-center text-xs font-black xl:block", rootAdminTheme ? "bg-emerald-400/10 text-emerald-100 ring-1 ring-emerald-300/15" : "bg-white text-[#74443A]")}>
          {rootAdminTheme ? "quản lý chính" : "make & photo"}
        </p>
      </div>

      {session ? (
        <div className={cn("mb-4 flex items-center justify-center gap-3 rounded-2xl border p-2 shadow-sm xl:mb-5 xl:justify-start xl:rounded-[1.4rem] xl:p-3", rootAdminTheme ? "border-emerald-300/15 bg-[#06140D] text-slate-100" : "border-[#F4C7C4] bg-white")}>
      ...
      <nav className="space-y-4">
        {visibleNavGroups.map((group) => (
          <div key={group.title}>
            <p className={cn("mb-2 hidden px-2 text-xs font-black uppercase tracking-wide xl:block", rootAdminTheme ? "text-slate-500" : "text-[#C17D8A]")}>{group.title}</p>
      ...
      <div className={cn("mt-5 border-t pt-4", rootAdminTheme ? "border-emerald-300/15" : "border-[#F4C7C4]")}>
        <Button variant="ghost" className={cn("w-full justify-center xl:justify-start", rootAdminTheme ? "text-slate-400 hover:bg-emerald-400/8 hover:text-emerald-100" : "")} aria-label="Cai dat">
          <Settings size={18} />
