...
const navGroups: Array<{ title: string; items: NavItem[] }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "booking", label: "Booking", icon: CalendarDays, href: "/booking" },
...
export function Sidebar({ session }: { session: CurrentSession | null }) {
  ...
  function classes(active: boolean) {
    return cn(
      ...
      active ? "bg-white text-[#5B342C] shadow-sm ring-1 ring-[#F4C7C4]" : "text-[#9B746B] hover:bg-white/75 hover:text-[#5B342C]",
    );
  }
  ...
  return (
    <aside className="hidden w-72 shrink-0 border-r border-[#F4C7C4] bg-[#FFE8E8]/70 px-4 py-5 lg:block">
      <div className="mb-5 rounded-[1.8rem] border-4 border-[#F7AFC0] bg-[#FFF9EF] p-4 text-[#5B342C] shadow-sm">
        ...
        <p className="mt-3 rounded-full bg-white px-3 py-2 text-center text-xs font-black text-[#74443A]">make & photo</p>
      </div>
      ...
      {session ? (
        <div className="mb-5 flex items-center gap-3 rounded-[1.4rem] border border-[#F4C7C4] bg-white p-3 shadow-sm">
          ...
          <div className="min-w-0">
            <p className="whitespace-normal break-words text-sm font-black leading-5 text-[#5B342C]">{session.user.name}</p>
            <p className="whitespace-normal break-words text-xs font-semibold leading-4 text-[#9B746B]">{session.user.email}</p>
          </div>
        </div>
      ) : null}
      ...
      <nav className="space-y-4">
        {navGroups.map((group) => (
          <div key={group.title}>
            <p className="mb-2 px-2 text-xs font-black uppercase tracking-wide text-[#C17D8A]">{group.title}</p>
            <div className="space-y-1">
              {group.items
                .filter((item) => !item.adminOnly || role === "ADMIN" || role === "MANAGER")
                .map((item) => {
                  const Icon = item.icon;
                  const active = item.href ? pathname === item.href && activeResource === item.id : activeResource === item.id && pathname === "/";
                  const content = (
                    <>
                      <Icon size={18} />
