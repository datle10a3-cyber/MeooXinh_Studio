...
export const Sidebar = memo(function Sidebar({ session, rootAdminTheme = false }: { session: CurrentSession | null; rootAdminTheme?: boolean }) {
  ...
  return (
    <aside className={cn("studio-sidebar studio-gpu hidden shrink-0 overflow-y-auto px-2 py-4 md:flex md:w-20 md:flex-col lg:w-56 lg:px-4 lg:py-5 xl:w-64 xl:py-6", rootAdminTheme ? "border-r border-emerald-300/15 bg-[#04110A]" : "border-r border-[#F4C7C4]/50 bg-white/50")}>
      <div className={cn("mb-4 rounded-2xl p-2 shadow-[0_8px_20px_rgba(184,95,108,0.06)] lg:mb-5 lg:rounded-[1.5rem] lg:p-3 xl:mb-6 xl:rounded-[2rem] xl:p-5", rootAdminTheme ? "border border-emerald-300/20 bg-[#06140D] text-slate-100 shadow-[0_18px_50px_rgba(2,6,23,0.28)]" : "border-2 border-[#F7AFC0] bg-white text-[#5B342C]")}>
        {rootAdminTheme ? (
        ...
        <p className={cn("mt-2 hidden rounded-full px-2 py-1.5 text-center text-xs font-black lg:block", rootAdminTheme ? "bg-emerald-400/10 text-emerald-100 ring-1 ring-emerald-300/15" : "bg-white text-[#74443A]")}>
          {rootAdminTheme ? "quản lý chính" : "make & photo"}
        </p>
      </div>

      {session ? (
        <div className={cn("mb-4 flex items-center justify-center gap-3 rounded-2xl border p-2 shadow-sm lg:mb-4 lg:justify-start lg:rounded-[1.4rem] lg:p-3 xl:mb-5", rootAdminTheme ? "border-emerald-300/15 bg-[#06140D] text-slate-100" : "border-[#F4C7C4] bg-white")}>
          <div className={cn("grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-2xl font-black text-white lg:h-11 lg:w-11", rootAdminTheme ? "border border-emerald-300/25 bg-emerald-400/10 text-emerald-100" : "bg-[#EA7188]")}>
            {rootAdminTheme ? (
              <ShieldCheck size={18} />
